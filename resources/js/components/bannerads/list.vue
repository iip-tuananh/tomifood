        ...
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <div v-if="!isReady || list.length === 0" class="text-center p-4">
                <span v-if="!isReady">Đang tải...</span>
                <span v-else-if="list.length === 0">Không có dữ liệu</span>
              </div>
              <vs-table v-if="isReady && list.length > 0" stripe :data="list" max-items="10" pagination>
                <template slot="thead">
                  <vs-th>Ảnh</vs-th>
                  <vs-th>Tên</vs-th>
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    <vs-td :data="tr.id">
                      <vs-avatar v-if="tr.image" size="70px" :src="tr.image" />
                    </vs-td>
                    <vs-td :data="tr.name">
                      {{ tr.content ? (typeof tr.content === 'string' ? JSON.parse(tr.content)[0]?.content || '' : '') : '' }}
                    </vs-td>
                    <vs-td >
